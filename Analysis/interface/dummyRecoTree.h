...
// Fri Jan 22 15:33:16 2016 by ROOT version 5.34/18
// from TTree recoTree/recoTree
// found on file: analysisTrees_V10/SiPM_runs.root
//////////////////////////////////////////////////////////

#ifndef dummyRecoTree_h
#define dummyRecoTree_h
...
public :
   ...
   Int_t           nFibres_hodoX1[31];   //[nClusters_hodoX1]
   Float_t         pos_hodoX1[31];   //[nClusters_hodoX1]
   Float_t         pos_corr_hodoX1[31];   //[nClusters_hodoX1]
   Int_t           nClusters_hodoY1;
   Int_t           nFibres_hodoY1[25];   //[nClusters_hodoY1]
   Float_t         pos_hodoY1[25];   //[nClusters_hodoY1]
   Float_t         pos_corr_hodoY1[25];   //[nClusters_hodoY1]
   Int_t           nClusters_hodoX2;
   Int_t           nFibres_hodoX2[24];   //[nClusters_hodoX2]
   Float_t         pos_hodoX2[24];   //[nClusters_hodoX2]
   Float_t         pos_corr_hodoX2[24];   //[nClusters_hodoX2]
   Int_t           nClusters_hodoY2;
   Int_t           nFibres_hodoY2[22];   //[nClusters_hodoY2]
   Float_t         pos_hodoY2[22];   //[nClusters_hodoY2]
   Float_t         pos_corr_hodoY2[22];   //[nClusters_hodoY2]
   ...
   dummyRecoTree(TTree *tree=0);
   virtual ~dummyRecoTree();
...
#ifdef dummyRecoTree_cxx
dummyRecoTree::dummyRecoTree(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("analysisTrees_V10/SiPM_runs.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("analysisTrees_V10/SiPM_runs.root");
      }
...
dummyRecoTree::~dummyRecoTree()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t dummyRecoTree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t dummyRecoTree::LoadTree(Long64_t entry)
{
...
void dummyRecoTree::Init(TTree *tree)
{
...
void dummyRecoTree::Show(Long64_t entry)
{
...
Int_t dummyRecoTree::Cut(Long64_t entry)
{
