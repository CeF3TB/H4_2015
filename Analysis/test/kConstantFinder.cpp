...
int main( int argc, char* argv[] ) {
  ...
  TFile* file = TFile::Open(fileName.c_str());
   ...
   TVectorD resValueChInt(NSTEPS);
   TVectorD resErrValueChInt(NSTEPS);
   TH1F*   totalHistos_tight_maxAmpl_fit_tot[NSTEPS];  
   TH1F*   totalHistos_tight_chInt_tot[NSTEPS];  
   ...
   for(int i= 0; i< NSTEPS;i++){
     ...
     totalHistos_tight_chInt_tot[i]= new TH1F ("histo_step_chInt_"+step,"",200*4,0,500000*4);   
	...
	if(TMath::Abs(0.5* (t.cluster_pos_corr_hodoX1+t.cluster_pos_corr_hodoX2))< 3 && TMath::Abs( 0.5* (t.cluster_pos_corr_hodoY1+t.cluster_pos_corr_hodoY2))< 3 && (t.wc_x_corr-t.cluster_pos_corr_hodoX2)<4 && (t.wc_y_corr-t.cluster_pos_corr_hodoY2)< 4  && TMath::Abs( (t.cluster_pos_corr_hodoX1-t.cluster_pos_corr_hodoX2))<1.5 &&TMath::Abs( (t.cluster_pos_corr_hodoY1-t.cluster_pos_corr_hodoY2))<1.5){
	    ...
	    totalHistos_tight_chInt_tot[i]->Fill(t.cef3_chaInt_wls->at(0)+t.cef3_chaInt_wls->at(1)+(i*0.1)*t.cef3_chaInt->at(2)+t.cef3_chaInt_wls->at(3));
	}
      }
     }
     ...
     totalHistos_tight_chInt_tot[i]->Write();

     //fitting with Cruijff
     if(totalHistos_tight_maxAmpl_fit_tot[i]){
       TH1F* histo;
     ...
     if(totalHistos_tight_chInt_tot[i]){
       TH1F* histo;
       histo=totalHistos_tight_chInt_tot[i];
       ...
       if(resoErr<0.2){//avoid points where fit didn't converge
	 resValueChInt[i]=reso;
	 resErrValueChInt[i]=resoErr;
       }

       cans->Write("c_chInt_"+step);
     }
   ...
   c1.Clear();
   TGraphErrors* resGraphChInt = new TGraphErrors(cValue,resValueChInt,cValueErr,resErrValueChInt);
   resGraph->SetName("resGraphChInt");
   resGraphChInt->SetMarkerColor(kRed);
   resGraphChInt->SetLineColor(kRed);
   resGraphChInt->SetFillColor(kRed);
   resGraphChInt->SetMarkerStyle(kRed);
   resGraphChInt->GetXaxis()->SetTitle("C_{2}");
   resGraphChInt->GetYaxis()->SetTitle("resolution [%]");
   resGraphChInt->GetYaxis()->SetRangeUser(2.3,4.8);
   resGraphChInt->Draw("AP");
   c1.SaveAs("resScan_chInt.png");
   c1.SaveAs("resScan_chInt.pdf");
   resGraphChInt->Write();

   resValueChInt.Write("resValueChInt");
   resErrValueChInt.Write("resValueErrChInt");
